...
const AdminGroceryApproval = () => {
  ...
  useEffect(() => {         
    const fetchData = async () => {
      try {
        const response = await fetch(`https://lmarttestapi-ctajf3hqfddkgebw.centralindia-01.azurewebsites.net/api/UploadGrocery/GetGroceryItems/${id}`);
  ...
  const handleSubmit = async () => {
    ...
    try {   
      const response = await fetch(`https://lmarttestapi-ctajf3hqfddkgebw.centralindia-01.azurewebsites.net/api/UploadGrocery/UpdateGroceryItems?id=${id}`, {
        method: "PUT",
