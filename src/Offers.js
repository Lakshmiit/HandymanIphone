...
useLayoutEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`https://lmarttestapi-ctajf3hqfddkgebw.centralindia-01.azurewebsites.net/api/Product/GetAllProductList`);
    ...
    (async () => {
      ...
      try {
        ...
        const { data } = await axios.get(
          `https://lmarttestapi-ctajf3hqfddkgebw.centralindia-01.azurewebsites.net/api/Product/GetProductsByCategory?Category=${encodeURIComponent(decoded)}`
        );
    ...
    limiterRef.current(async () => {
      try {
        const res = await fetch(
          `https://lmarttestapi-ctajf3hqfddkgebw.centralindia-01.azurewebsites.net/api/FileUpload/download?generatedfilename=${encodeURIComponent(first)}`
        );
